...
#######################################################################################
### ComplexCell Class: Computes the responses of COMPLEX cells based on the sum of simple
###                    cell responses which are computed and summed inside the class
###                    much of the code here is a duplicate from the one of the SimpleCell class.
#######################################################################################

class ComplexCell(object):
    """
        Computes and stores the complex cell responses as well as the different filters needed to
        create those resposnes. The main method is `compute_responses`, which
        takes the image, convolves it with all the filters, and compute the normalisation required
        by the model of the Simple Cell and finally sums the different phases to get the complex cell 
        output that we expect. Then, the method `get_response` returns the response from
        a particular filter. 
    ...
    def __init__(self, im_shape, angles, velocities, wavelengths, phases, c, alpha, a_1, s_1, K, 
                 deactivation_rate = 0.0, filt_width = 0.56, directory_cell = 'comp_cell', file_cell = 'response' , 
        ...
        #self.file_filt = file_filt # name of the file to which the filter will be stored
        #self.directory_filt = directory_filt # directory in which the filter will be stored
...
###################################################################################################                                   
    ...
    def compute_responses(self, stimulus):
        """
           Computes the responses of ComplexCells to a stimulus given as input, which has to be of the
        ...
        #if not os.path.isdir(self.directory_filt):
        #    os.mkdir(self.directory_filt)
        ...
        if self.deactivation_rate > 0:
            self.deactivation_map_simp = np.random.choice([0, 1], size=(self.N_filters,self.im_shape[1], self.im_shape[2]), 
                                                  p=[ self.deactivation_rate, 1-self.deactivation_rate])
            self.deactivation_map_comp = np.random.choice([0, 1], size=(int(self.N_filters/4),
                                                                        self.im_shape[1], self.im_shape[2]), 
                                                  p=[ self.deactivation_rate, 1-self.deactivation_rate])
    ...
    def response_parallel(self, angles_split, velocities_split):
        """
            Creates and saves a bank of 3D spatiotemporal Gabor Filters from the parameters given as input
            as well as from the class. Also computes the raw responses to the stimulus given in 
            `compute_responses` and stores those as well.
        ...
        #print(os.getpid(),(angles_split), (velocities_split), verbose)
        normalisation = np.zeros(self.stimulus.shape)
        
        for i, [theta, v] in enumerate(zip(angles_split, velocities_split)):
            # Stores all responses for all lambdas and phis at the same time
            response_sum = np.zeros(self.stimulus.shape)
            for j, lambd in enumerate(self.wavelengths):
                ...
                for k, phi in enumerate(self.phases):
                    pos = O*P*i + P*j + k
                        ...
                        #To keep the same shape of Gabor Filter for all lambdas
                    ...
                    if self.deactivation_rate >0:
                        response_sum += self.deactivation_map_simp[pos]*response 
                        # summing the responses with the same theta, psi. 
                    else:
                        response_sum += response 
            
            if self.deactivation_rate >0: 
                response_sum *= self.deactivation_map_comp[i]
            response_path = self.get_resp_path(theta, v)
            np.save(response_path, response_sum)  
            
            if self.verbose:
                print(str(os.getpid()).ljust(8), (str(pos+1)+'/'+str(len(angles_split)*O*P)).ljust(8), 
                      str(int(theta*180/np.pi)).ljust(5), str(round(v,3)).ljust(7), 'Sum resp.'.ljust(11), '|',
                      str(round(np.sum(response_sum[response_sum > 0]),3)).ljust(10),
                      str(round(np.sum(response_sum[response_sum < 0]), 3)).ljust(10), 
                      str(round(np.sum(response_sum), 3)).ljust(6), 
                      str(round(response_sum.max(),3)).ljust(6), str(round(response_sum.min(), 3)).ljust(6),
                      str(round(np.sum(np.abs(response_sum)), 3)).ljust(6))
                
            normalisation += response_sum
...
###     3. get_filt_path: Constructs the path to the file for a filter from the parameters theta, v, lambd, phi
################################################################################################### 
    ...
    def get_resp_path(self, *args):
        ...
        if len(args) == 1 and isinstance(args[0], (int,np.int_)):
            theta, v, _, _ = self.filter_map_rad[args[0]]
        elif len(args) == 2 and all(isinstance(arg, (float,int,np.int_,np.float_)) for arg in args):
            theta, v = args[0], args[1]
        else:
            raise TypeError('Wrong type of input for the method get_filter. '+
                            'Please make sure you either pass an integer or '+ 
                            'both parameters uniquely identifying the desired Complex cell response.')
        ...
        theta = str(int(theta * 180 / np.pi)); v_str = self.int_to_str(v)        
